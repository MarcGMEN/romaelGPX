...
while (strlen($map_string) > 1800);  // Google Map web service URL limit is 2048 characters. 1800 is arbitrary attempt to stay under 2048
    ...
    echo "LatLng_start $LatLng_start";
...
<body>
<pre>
	<?php //print_r($pFFA->data_mesgs)?>
	<?php print_r($pFFA->data_mesgs['file_id'])?>
...
?>
        ...
        <dt>Distance: </dt>
        <dd><?php echo $pFFA->data_mesgs['session']['total_distance']; ?> km</dd>
      </dl>
    </div>
  </div>
  <div class="col-md-2">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="col-md-12">
            <?php  print_r($pFFA->data_mesgs['session']);?>
            max speed <?=$pFFA->data_mesgs['session']['max_speed']?>
            avg speed <?=$pFFA->data_mesgs['session']['avg_speed']?>
              <div id="speed" style="width:100%; height:100px; margin-bottom:8px"></div>
            max heart<?=$pFFA->data_mesgs['session']['max_heart_rate']?>
            avg heart<?=$pFFA->data_mesgs['session']['avg_heart_rate']?>
              <div id="heart_rate" style="width:100%; height:100px; margin-bottom:8px"></div>
            max cadence<?=$pFFA->data_mesgs['session']['max_cadence']?>
            avg cadence<?=$pFFA->data_mesgs['session']['avg_cadence']?>
              <div id="cadence_rate" style="width:100%; height:100px; margin-bottom:8px"></div>
            Ascension<?=$pFFA->data_mesgs['session']['total_ascent']?>
            Descente<?=$pFFA->data_mesgs['session']['total_descent']?>
              <div id="altitude_rate" style="width:100%; height:100px; margin-bottom:8px"></div>
              <div id="temperature" style="width:100%; height:100px; margin-bottom:8px"></div>
            </div>
          </div>
        </div>
      </div>
...
<pre>
<?php print_r($pFFA->data_mesgs['record']['speed']);?>
</pre>	
...
?>
	...
	var cadence_rate_options = {
		      lines: { show: true, fill: true, fillColor: 'rgba(255, 255, 0, .4)', lineWidth: 1 },
		      points: { show: false },
		      xaxis: {
		        show: false
		      },
...
<?php
    unset($tmp);
    $tmp = [];
foreach ($pFFA->data_mesgs['record']['cadence'] as $key => $value) {
    $tmp[] = '['.$key.', '.$value.']';
}
    echo implode(', ', $tmp);
?>
	...
	var altitude_rate_options = {
		      lines: { show: true, fill: true, fillColor: 'rgba(255, 0, 128, .4)', lineWidth: 1 },
		      points: { show: false },
		      xaxis: {
		        show: false
		      },
...
<?php
    unset($tmp);
    $tmp = [];
foreach ($pFFA->data_mesgs['record']['altitude'] as $key => $value) {
    $tmp[] = '['.$key.', '.$value.']';
}
    echo implode(', ', $tmp);
?>
	...
	var temperature_options = {
		      lines: { show: true, fill: false, fillColor: 0, lineWidth: 1 },
		      points: { show: false },
		      xaxis: {
		        show: false
		      },
...
<?php
  unset($tmp);
  $tmp = [];
foreach ($pFFA->data_mesgs['record']['temperature'] as $key => $value) {
  $tmp[] = '['.$key.', '.$value.']';
}
  echo implode(', ', $tmp);
?>
	...
	$.plot('#cadence_rate', [cadence_rate], cadence_rate_options);
	$.plot('#altitude_rate', [altitude_rate], altitude_rate_options);
	$.plot('#temperature', [temperature], temperature_options);
  });
</script>
</body>
</html>